...
preds <- urban_table %>% 
  ...
  mutate(predoutcome = predlean + 1.7)
...
polls <- tibble(
  ...
  avg = c(-1.8, -7, 0.3, -1.7, 0.0, 0.0, -0.5, -0.7, 24.7, -19, -6.4, -5.3, 29.8, 28.5, 6.5, 7.6, 9, -5, -27, -10, -7, -16.5, 18.5, 13.5, 11, 7.6, 16, 16.5, 13, -11.5, -15, 20.6, -16, -17, -27, -16, 41, 12, -25.4, 29.4, -30.7, 16.9, -14.6, -26, -18.6, -16.5, 15.8, -25.8, -21, -43.3)
)
...
However, in reality this is a much closer contest than it appears: a shift of just 1.7 points to the right nationally (exactly Kamala Harris' edge as of today) would flip Minnesota, Texas and, in the height of irony, Vermont, giving Trump exactly the 270 electoral votes needed to be the next President of the United States. If we ignore Vermont, a 2020-sized polling error of roughly 4 points in Trump's favor would be sufficient for him to retake Pennsylvania and Florida, as well as Maine's 1st district for good measure, enough for a comfortable Electoral College victory.
...
new_bounds <- new_bounds %>% 
  mutate(pred_cat = case_when(
    ...
    prediction < -4 & prediction >= -10 ~ "Lean R",
    prediction >= -4 & prediction <= 4 ~ "Tossup",
    prediction > 4 & prediction <= 10 ~ "Lean D",
